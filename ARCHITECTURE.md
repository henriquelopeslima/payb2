...
Este documento descreve as decisões de arquitetura e os porquês por trás do desenho do PayB2, uma API de Transferências Financeiras baseada em Symfony, Clean Architecture e princípios de DDD.
...
- **Infrastructure**
  ...
  - Limite: toda dependência de framework/IO/observabilidade vive aqui.
...
### Problema
Autowiring do `TracerProviderInterface` falhava sem um serviço concreto.

### Solução
- Ambientes com tracing real:
  - Registrar `OpenTelemetry\SDK\Trace\TracerProvider` via **Factory** (ex.: `Bootstrap::init()`), que cria `SpanExporter` OTLP e `BatchSpanProcessor`.
  - Alias: `OpenTelemetry\API\Trace\TracerProviderInterface: '@OpenTelemetry\SDK\Trace\TracerProvider'`.

- Ambientes sem OTel (desenvolvimento mínimo):
  - Usar `OpenTelemetry\API\Trace\NoopTracerProvider` e aliasar a interface para o noop.
